...
"""Document state extraction from FreeCAD documents.

This module provides functionality for extracting the document tree structure
from a live FreeCAD document and converting it to pure domain models.
"""
...
from freecad.diff_wb.ports.freecad_port import FreeCadPort, get_port
...
def _extract_property_value(obj: object, prop_name: str) -> PropertyValue | None:
    """Extract a single property value from a FreeCAD object.

    Delegates to PropertyValue.from_freecad_property() which handles
    ...
    Returns:
        A PropertyValue if successful, None if the property couldn't be read
    """
    try:
        value = getattr(obj, prop_name)
        expression = _get_expression_for_property(obj, prop_name)
        return PropertyValue.from_freecad_property(prop_name, value, expression=expression)
...
def extract_tree(ctx: FreeCadContext | None = None) -> Snapshot:
    ...
    Returns:
        ...
        Returns an empty Snapshot if no document is open or extraction fails.

    Example:
        >>> snapshot = extract_tree()
        >>> print(f"Found {len(snapshot.root_nodes)} root objects")
        >>> for node in snapshot.get_all_nodes():
        ...     print(f"  {node.path} ({node.type_id})")
    """
    port = get_port(ctx)
    ...
    try:
        ...
        for obj in objects:
            if not hasattr(obj, "Name"):
                ...
                port.warn("Skipping object without Name attribute")
                continue
        ...
        port.warn(f"Error extracting document tree: {e}")
